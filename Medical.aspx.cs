...
namespace Pets_Heaven_
{
    public partial class Medical : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                LoadMedicalProducts(page);
            }
        }

        private void LoadMedicalProducts(int pageIndex)
        {
            ...
            using (SqlConnection con = new SqlConnection(conStr))
            {
                string query = @"
                    SELECT * FROM (
                        ...
                        FROM Products 
                        WHERE CategoryID = 4 AND ProductName LIKE @Search
                    ) AS Paged
                    WHERE RowNum BETWEEN @Start + 1 AND @Start + @PageSize;

                    SELECT COUNT(*) FROM Products WHERE CategoryID = 4 AND ProductName LIKE @Search;
                ";
                ...
                rptMedical.DataSource = ds.Tables[0];
                rptMedical.DataBind();
        ...
        private void GeneratePaging(int totalRecords, int currentPage)
        {
            ...
            {
                string css = i == currentPage ? "class='current'" : "";
                sb.Append($"<a href='Medical.aspx?page={i}' {css}>{i}</a>");
            }

            if (currentPage < totalPages)
            {
                sb.Append($"<a href='Medical.aspx?page={currentPage + 1}'>&raquo; Next</a>");
            }
    ...
    "Swal.fire('Success!', 'Item added to cart successfully!', 'success');", true);
