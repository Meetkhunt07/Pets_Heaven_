...
namespace Pets_Heaven_
{
    public partial class ManageAccessories : System.Web.UI.Page
    {
        ...
        private void LoadAccessories(string search = "")
        {
            using (SqlConnection con = new SqlConnection(conStr))
            {
                string query = "SELECT * FROM Products WHERE CategoryID = 2";
        ...
        {
            LoadAccessories(txtSearch.Text.Trim());
        }
        ...
        {
            GridViewRow row = gvAccessories.Rows[e.RowIndex];
            int productId = Convert.ToInt32(gvAccessories.DataKeys[e.RowIndex].Value);
            ...
            if (fuImage.HasFile)
            {
                string folderPath = Server.MapPath("~/Images/Accessories/");
                if (!Directory.Exists(folderPath)) Directory.CreateDirectory(folderPath);

                string fileName = Path.GetFileName(fuImage.FileName);
                string savePath = Path.Combine(folderPath, fileName);
                fuImage.SaveAs(savePath);
                imageUrl = "Images/Accessories/" + fileName;
            }
            ...
            litScript.Text = "<script>Swal.fire('Updated!','Accessory updated successfully.','success');</script>";
        }
        ...
        protected void gvAccessories_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            ...
            litScript.Text = "<script>Swal.fire('Deleted!','Accessory deleted successfully.','success');</script>";
        }
    }
}
